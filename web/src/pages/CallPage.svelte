...
<div class="call-page">
  <section class="video-column" aria-label="Vivid call">
    ...
    <VideoGrid localStream={media.localStream} username={session.username} deviceType={session.deviceType} microphoneMuted={media.microphoneMuted} noiseCancellationEnabled={media.noiseCancellationEnabled} cameraStopped={media.cameraStopped} cameraFacing={media.cameraFacing} screenSharing={media.screenSharing} displayStream={media.displayStream} peers={session.peers} onTogglePeerPlayback={session.onTogglePeerPlayback} />
  </section>
